...
class DiagnosisController extends Controller
{
    ...
    public function storeData(Request $request)
    {
        ...
        return redirect()->route('diagnosa.gejala')->with('success', 'Data berhasil disimpan. Silakan lanjutkan ke tahap berikutnya.');
    }

    public function showGejalaForm()
    {
        // Cek apakah data sudah ada di session
        if (!session()->has('nama_ibu') || !session()->has('nama_anak')) {
            return redirect()->route('diagnosa.data')->with('error', 'Silakan isi data diri terlebih dahulu.');
        }
    ...
    public function prosesDiagnosa(Request $request)
    {
        // Cek session data
        if (!session()->has('nama_ibu') || !session()->has('nama_anak')) {
            return redirect()->route('diagnosa.data')->with('error', 'Session expired. Silakan mulai dari awal.');
        }
    ...
    // Method untuk reset session jika diperlukan
    public function resetSession()
    {
        session()->forget(['nama_ibu', 'nama_anak', 'usia_anak', 'jenis_kelamin', 'tanggal_lahir', 'alamat', 'jenis_vaksin', 'tempat_imunisasi', 'tanggal_imunisasi', 'gejala_dipilih']);
        return redirect()->route('diagnosa.data')->with('info', 'Sesi telah direset. Silakan mulai dari awal.');
    }
}
