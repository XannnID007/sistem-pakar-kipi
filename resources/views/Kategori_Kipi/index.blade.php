        ...
        <tbody>
            @foreach($kategori as $item)
                <tr>
                    ...
                    <td>{{ $item->saran }}</td>
                    <td>
                        <a href="{{ route('pakar.kategori_kipi.edit', $item->id) }}" class="btn btn-warning btn-sm">
                        <i class="fas fa-edit"></i> Edit</a>

                        <form action="{{ route('pakar.kategori_kipi.destroy', $item->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin ingin hapus?')">
                            @csrf
                            @method('DELETE')
